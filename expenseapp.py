...
def add_expense(username, expense_date, amount, category, note):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO expenses (username, date, amount, category, note) VALUES (?, ?, ?, ?, ?)",
              (username, str(expense_date), amount, category, note))
    conn.commit()
    conn.close()
...
if not st.session_state.logged_in:
    st.markdown('<div class="app-title">💸 Spendwise</div>', unsafe_allow_html=True)
    st.markdown('<div class="app-subtitle">Track where your money actually goes.</div>', unsafe_allow_html=True)
...
with top_left:
    st.markdown('<div class="app-title">💸 Spendwise</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="app-subtitle">Welcome back, {st.session_state.username}.</div>', unsafe_allow_html=True)
with top_right:
    st.write("")
...
with left:
    ...
    with st.form("add_expense_form", clear_on_submit=True):
        ...
        if submitted and amount > 0:
            add_expense(st.session_state.username, expense_date, amount, category, note)
            st.success("Added!")
            st.rerun()
...
with right:
    ...
    if not df.empty:
        ...
        colors = [CATEGORY_META.get(c, {}).get("color", "#A3A3A3") for c in by_cat["category"]]
        ...
        fig.update_layout(
            showlegend=True,
            legend=dict(orientation="h", y=-0.15, font=dict(family="Inter", size=12)),
            margin=dict(t=10, b=10, l=10, r=10), height=320,
            paper_bgcolor="rgba(0,0,0,0)",
            annotations=[dict(text=f"₹{total_spent:,.0f}", x=0.5, y=0.5,
                               font=dict(family="JetBrains Mono", size=18, color="#1B4D3E"),
                               showarrow=False)]
        )
...
if not df.empty:
    ...
    for _, row in recent.iterrows():
        meta = CATEGORY_META.get(row["category"], {"icon": "✨", "color": "#A3A3A3"})
...
st.write("")
st.markdown("#### Day-wise total spending")
if not df.empty:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    daily_totals = df.groupby(df["date"].dt.date)["amount"].sum().sort_index(ascending=False)
    for day, total in daily_totals.items():
        st.markdown(f"""
        <div class="txn-row">
            <div class="txn-left">
                <div class="txn-icon">📅</div>
                <div class="txn-note">{day.strftime('%A, %d %b %Y')}</div>
            </div>
            <div class="txn-amount">₹{total:,.0f}</div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
else:
    st.info("No transactions yet — day-wise totals will show up here.")
